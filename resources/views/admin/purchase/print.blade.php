...
<head>
    ...
    <style>
        ...
        /* Chrome print needs exact background colors preserved */
        * {
...
<body class="text-gray-800 text-sm p-8">

<!-- Header -->
<div class="flex items-start justify-between border-b border-gray-300 pb-6 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ config('app.name') }}</h1>
        <p class="text-xs text-gray-500 mt-1">Pharmacy Management System</p>
    </div>
    <div class="text-right">
        <h2 class="text-lg font-semibold text-blue-600">PURCHASE ORDER</h2>
        <p class="text-xs text-gray-500 mt-1">{{ $po->po_number }}</p>
    </div>
</div>
...
<div class="grid grid-cols-2 gap-6 mb-8">
    <div>
        <h3 class="text-xs uppercase text-gray-400 font-semibold mb-1">Supplier</h3>
        <p class="font-medium text-gray-800">{{ $po->supplier->name ?? 'N/A' }}</p>
        @if($po->supplier?->address)
            <p class="text-xs text-gray-500 mt-0.5">{{ $po->supplier->address }}</p>
        @endif
        @if($po->supplier?->phone)
            <p class="text-xs text-gray-500">{{ $po->supplier->phone }}</p>
        @endif
    </div>
    <div class="text-right">
        <div class="grid grid-cols-2 gap-x-4 text-xs">
            <span class="text-gray-400">Order Date</span>
            <span class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($po->order_date)->format('d M Y') }}</span>

            <span class="text-gray-400">Expected Delivery</span>
            <span class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($po->expected_delivery_date)->format('d M Y') ?? '-' }}</span>

            <span class="text-gray-400">Status</span>
            <span class="font-medium text-gray-800 capitalize">{{ str_replace('_', ' ', $po->status) }}</span>

            <span class="text-gray-400">Prepared By</span>
            <span class="font-medium text-gray-800">{{ $po->createdBy->name ?? '-' }}</span>
        </div>
    </div>
</div>
...
<table class="w-full text-xs border-collapse mb-6">
    ...
    <tr class="bg-gray-100 text-gray-600 uppercase text-[10px]">
        <th class="text-left px-3 py-2 border border-gray-200">#</th>
        <th class="text-left px-3 py-2 border border-gray-200">Product</th>
        <th class="text-center px-3 py-2 border border-gray-200">Qty</th>
        <th class="text-right px-3 py-2 border border-gray-200">Unit Cost</th>
        <th class="text-right px-3 py-2 border border-gray-200">Total</th>
    </tr>
    </thead>
    <tbody>
    @foreach($po->items as $index => $item)
        <tr>
            <td class="px-3 py-2 border border-gray-200 text-gray-500">{{ $index + 1 }}</td>
            <td class="px-3 py-2 border border-gray-200 font-medium">{{ $item->product->name ?? 'N/A' }}</td>
            <td class="px-3 py-2 border border-gray-200 text-center">{{ number_format($item->quantity_ordered, 2) }}</td>
            <td class="px-3 py-2 border border-gray-200 text-right">৳ {{ number_format($item->unit_cost, 2) }}</td>
            <td class="px-3 py-2 border border-gray-200 text-right font-medium">৳ {{ number_format($item->total_price, 2) }}</td>
        </tr>
    @endforeach
    </tbody>
</table>
...
<div class="flex justify-end mb-10">
    <div class="w-64 text-xs space-y-1.5">
        <div class="flex justify-between text-gray-500">
            <span>Subtotal</span>
            <span>৳ {{ number_format($po->sub_total, 2) }}</span>
        </div>
        <div class="flex justify-between text-gray-500">
            <span>Discount</span>
        ...
        <div class="flex justify-between font-bold text-gray-900 text-sm border-t border-gray-300 pt-2 mt-1">
            <span>Grand Total</span>
...
@if($po->notes)
    <div class="mb-10">
        <h3 class="text-xs uppercase text-gray-400 font-semibold mb-1">Notes</h3>
        <p class="text-xs text-gray-600">{{ $po->notes }}</p>
    </div>
@endif
